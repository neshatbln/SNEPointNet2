...
import tf_util
import pc_util
from pointnet2_sem_seg3 import *

parser = argparse.ArgumentParser()
parser.add_argument('--num_gpus', type=int, default=3, help='How many gpus to use [default: 1]') #Multi
parser.add_argument('--log_dir', default='log2', help='Log dir [default: log]') #Neshat
parser.add_argument('--num_point', type=int, default=12288, help='Point number [default: 4096]') #MAJID
parser.add_argument('--max_epoch', type=int, default=101, help='Epoch to run [default: 50]')  #MAJID
...
parser.add_argument('--decay_step', type=int, default=807, help='Decay step for lr decay [default: 300000]')   #MAJID  (THIS IS 5 EPOCHs)
parser.add_argument('--decay_rate', type=float, default=0.70, help='Decay rate for lr decay [default: 0.5]')     #MAJID
parser.add_argument('--eval_area', type=int, default=4, help='Which area to use for eval, option: 1-6 [default: 6]') #NB
parser.add_argument('--test_area', type=int, default=5, help='Which area to use for test (ignored in training), option: 1-6 [default: 6]') #MAJID
FLAGS = parser.parse_args()


NUM_GPUS = FLAGS.num_gpus  #Multi
BATCH_SIZE = FLAGS.batch_size
assert(BATCH_SIZE % NUM_GPUS == 0)   #Multi
DEVICE_BATCH_SIZE = BATCH_SIZE // NUM_GPUS   #Multi
...
MAX_NUM_POINT = 16384   #MAJID
NUM_CLASSES = 3  #Neshat
...
room_filelist = [line.rstrip() for line in open('bridge_npy_hdf5_data/room_filelist.txt')]
...
eval_area = 'Area_'+str(FLAGS.eval_area)
test_area = 'Area_'+str(FLAGS.test_area)
train_idxs = []
test_idxs = []
for i,room_name in enumerate(room_filelist):
    if eval_area in room_name:
        test_idxs.append(i)
...
class Dataset():
    def __init__(self, npoints=NUM_POINT, split='train'):  #1024
...
def train():
    with tf.Graph().as_default():
        with tf.device('/cpu:0'): #NB
            pointclouds_pl, labels_pl, smpws_pl = placeholder_inputs(BATCH_SIZE, NUM_POINT)  #MAJID2
            is_training_pl = tf.placeholder(tf.bool, shape=())
            
            # Note the global_step=batch parameter to minimize. 
            # That tells the optimizer to helpfully increment the 'batch' parameter for you every time it trains.

            batch = tf.Variable(0, trainable=False)  #original
            ...
            accuracy_1 = tf.reduce_sum(tf.cast(correct, tf.float32)) / float(BATCH_SIZE) #NB
            tf.summary.scalar('accuracy = correct / (BATCH_SIZE*NUM_POINT) =', accuracy)
            tf.summary.scalar('accuracy_1 = correct / (BATCH_SIZE) =', accuracy_1) #NB
...
def get_batch_wdp(dataset, idxs, start_idx, end_idx): #MAJID2
    bsize = end_idx-start_idx
    batch_data = np.zeros((bsize, NUM_POINT, 10)) #neshat
...
def get_batch(dataset, idxs, start_idx, end_idx): #MAJID2
    bsize = end_idx-start_idx
    batch_data = np.zeros((bsize, NUM_POINT, 10)) #NB
    #batch_data = np.zeros((bsize, NUM_POINT, 7))
    batch_label = np.zeros((bsize, NUM_POINT), dtype=np.int32)
    batch_smpw = np.zeros((bsize, NUM_POINT), dtype=np.float32)
    for i in range(bsize):
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        cur_batch_size = end_idx - start_idx  #MAJID
        #batch_smpw = np.zeros((cur_batch_size, NUM_POINT), dtype=np.float32) #MAJID2
 ...
 #
  ...
  #                  tmp, _ = np.histogram(k, range(NUM_CLASSES+1))
 #                   labelweights += tmp
 #               labelweights = labelweights.astype(np.float32)
 #               labelweights = labelweights / np.sum(labelweights)
 #               labelweights = 1 / np.log(1.2 + labelweights)
                #smpw = dataset[idxs[i + start_idx]]
   #             batch_smpw[i, j] = labelweights[current_label[j]]
...
def eval_one_epoch(sess, ops, test_writer, epoch):
    ...
    loss_sum = 0
    h = 0 #MAJID
    label = [0 for _ in range(total_num_points)] #MAJID
    predicted = [0 for _ in range(total_num_points)] #MAJID
    ...
    for batch_idx in range(num_batches):
        ...
        #batch_smpw = np.ones((cur_batch_size, NUM_POINT), dtype=np.float32)  # MAJID2
        ...
        pred_val = np.argmax(pred_val, 2)
        #pred_label = np.squeeze(pred_val) #MAJID
        correct = np.sum(pred_val == current_label[start_idx:end_idx])
        total_correct += correct
        total_seen += (BATCH_SIZE*NUM_POINT)
        ...
        for i in range(start_idx, end_idx):
            for j in range(NUM_POINT):
                l = current_label[i, j]
                label[h] = current_label[i, j] #MAJID
                predicted[h] = pred_val[i - start_idx, j] #MAJID
                h += 1 #MAJID

...
DATA_PATH = os.path.join(ROOT_DIR, 'data', 'bridge')
g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, 'meta/class_names.txt'))]
g_class2label = {cls: i for i,cls in enumerate(g_classes)}

g_class2color = {'crack':	[0,255,0],
                 'spal':	[0,0,255],
                 'nondefect':	[255,0,255]} #3 classes
g_easy_view_labels = [0,1,2] #3 classes
...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    """ Convert original dataset files to data_label file (each line is XYZRGB,NormalVector,L).
        We aggregated all the points from each instance in the room.

    Args:
        anno_path: path to annotations. e.g. Area_1/office_2/Annotations/
        out_filename: path to save collected points and labels (each line is XYZRGB,NormalVector,L)
    ...
    for f in glob.glob(os.path.join(anno_path, '*.txt')):
        ...
        points_list.append(np.concatenate([points, labels], 1)) # Nx7 (modified to 10, normal vector is added)
    ...
    if file_format=='txt':
        ...
        for i in range(data_label.shape[0]):
           fout.write('%f %f %f %d %d %d %0.3f %0.3f %0.3f %d \n' % \
...
def point_label_to_obj(input_filename, out_filename, label_color=True, easy_view=False, no_wall=False):
	...
	input_filename: each line is X Y Z R G B Nx Ny Nz L
 in modified : input_filename: each line is X Y Z R G B Nx Ny Nz L
    ...
    for i in range(data.shape[0]):
        ...
        if no_wall and ((label[i] == 2) or (label[i]==0)):
            continue
        if label_color:
            fout.write('v %0.3f %0.3f %0.3f %d %d %d %0.3f %0.3f %0.3f\n' % \
                (data[i,0], data[i,1], data[i,2], color[0], color[1], color[2], data[i,6], data[i,7], data[i,8]))
        else:
            fout.write('v %0.3f %0.3f %0.3f %d %d %d %0.3f %0.3f %0.3f\n' % \
...
def room2blocks(data, label, num_point, block_size=0.20, stride=0.10,
                random_sample=False, sample_num=None, sample_aug=1):
    """ Prepare block training data.
    Args:
        data: N x 9 numpy array, 012 are XYZ in meters, 345 are RGB in [0,1],678 are NxNyNz
            assumes the data is shifted (min point is origin) and aligned
            (aligned with XYZ axis)
        label: N size uint8 numpy array from 0-2
    ...
    Returns:
        block_datas: K x num_point x 9 np array of XYZRGBNxNyNz, RGB is in [0,1]
    ...
    if not random_sample:
        ...
        for i in range(num_block_x):
            for j in range(num_block_z):
                ...
                zbeg_list.append(j*stride)
    else:
        num_block_x = int(np.ceil(limit[0] / block_size))
        num_block_z = int(np.ceil(limit[2] / block_size))
        if sample_num is None:
            sample_num = num_block_x * num_block_z * sample_aug
    ...
    for idx in range(len(xbeg_list)): 
       ...
       cond = xcond & zcond
       if np.sum(cond) < 5: # discard block if there are less than 5 pts.
           continue
...
def room2blocks_wrapper(data_label_filename, num_point, block_size=0.20, stride=0.10,
                        random_sample=False, sample_num=None, sample_aug=1):
...
def room2blocks_plus_normalized(data_label, num_point, block_size, stride,
                                random_sample, sample_num, sample_aug):
    """ room2block, with input filename and RGB preprocessing.
        for each block centralize XYZ, add normalized Y as 9 channels
    """
    #data = data_label[:,0:6] #ORG
    data = data_label[:,0:9] #Neshat
    data[:,3:6] /= 255.0
    label = data_label[:,-1].astype(np.uint8)
    ...
    new_data_batch_2 = np.zeros((data_batch.shape[0], num_point, 12))
    
    for b in range(data_batch.shape[0]):

        new_data_batch[b, :, 9] = data_batch[b, :, 1]/max_room_y
        new_data_batch_2[b, :, 9] = data_batch[b, :, 0]/max_room_x
        new_data_batch_2[b, :, 10] = data_batch[b, :, 1]/max_room_y
        new_data_batch_2[b, :, 11] = data_batch[b, :, 2]/max_room_z
        minx = min(data_batch[b, :, 0])
        miny = min(data_batch[b, :, 1])
        minz = min(data_batch[b, :, 2])
        data_batch[b, :, 0] -= (minx+block_size/2)
        data_batch[b, :, 1] -= (miny+block_size/2)
        data_batch[b, :, 2] -= (minz+block_size/2)
    new_data_batch[:, :, 0:9] = data_batch
    new_data_batch_2[:, :, 0:9] = data_batch
    #return new_data_batch, new_data_batch_2, label_batch          #for sem_seg
    return new_data_batch, label_batch							#for DATA PRE-PRo


def room2blocks_wrapper_normalized(data_label_filename, num_point, block_size=0.20, stride=0.10,
                                   random_sample=False, sample_num=None, sample_aug=1):
    if data_label_filename[-3:] == 'txt':
        data_label = np.loadtxt(data_label_filename)
    elif data_label_filename[-3:] == 'npy':
...
def room2samples(data, label, sample_num_point):
    ...
    Args:
        data: N x 6 numpy array, 012 are XYZ in meters, 345 are RGB in [0,1], 678 are NxNyNz
    ...
    Returns:
        sample_datas: K x sample_num_point x 10
                     numpy array of XYZRGBNxNyNzY', RGB is in [0,1]
    ...
    sample_datas = np.zeros((batch_num, sample_num_point, 10))
...
def room2samples_plus_normalized(data_label, num_point):
    """ room2sample, with input filename and RGB preprocessing.
        for each block centralize XYZ, add normalized Y as 9 channel
    """
    data = data_label[:,0:9]
    data[:,3:6] /= 255.0
    label = data_label[:,-1].astype(np.uint8)
...
def collect_bounding_box(anno_path, out_filename):
    ...
    Args:
        anno_path: path to annotations. e.g. Area_1/part_1/Annotations/
...
def collect_point_bounding_box(anno_path, out_filename, file_format):
    ...
    Args:
        ...
        out_filename: path to save instance bounding boxes for each point,
            plus the point's XYZRGBLNxNyNz
            each line is XYZRGBLNxNyNz offsetX offsetY offsetZ a b c,

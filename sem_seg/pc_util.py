...
def read_ply_xyzrgb(filename):
    ...
    with open(filename, 'rb') as f:
        ...
        vertices[:,6] = plydata['vertex'].data['XX']
        vertices[:,7] = plydata['vertex'].data['YY']
        vertices[:,8] = plydata['vertex'].data['ZZ']
    return vertices
...
def write_ply_color(points, labels, out_filename, num_classes=None):
    ...
    for i in range(N):
        ...
        fout.write('v %f %f %f %d %d %d %f %f %f\n' % (points[i,0],points[i,1],points[i,2],c[0],c[1],c[2], points[i,6],points[i,7],points[i,8])) #NB
    fout.close()
